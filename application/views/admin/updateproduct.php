<div class="container">
    ...
    <form id="form-product" action="<?= base_url() . 'admin/upprodvalue' ?>" method="post" enctype="multipart/form-data" data-oc-toggle="ajax">
        <ul class="nav nav-tabs" id="myTab" role="tablist">
            ...
            <li class="nav-item" role="presentation">
                <button class="nav-link" id="option-tab" data-toggle="tab" data-target="#optionview" type="button" role="tab" aria-controls="option" aria-selected="false">Option</button>
            </li>
            <li class="nav-item" role="presentation">
                ...
                <div id="tab-attribute" class="tab-pane active">
                    <div class="table-responsive">
                        <table id="product-attribute" class="table table-bordered table-hover">
                            ...
                            <tbody>
                                <?php foreach ($productattribute as $pa) :
                                ?>
                                    <tr id="attribute-row-<?= $pa['id'] ?>" class="<?= $pa['id'] ?>">
                                        <td class="text-start">

                                            <input type="text" name="product_attribute[<?= $pa['id'] ?>][name]" value="<?= $pa['attributename'] ?>" placeholder="Attribute" id="input-attribute-<?= $pa['id'] ?>" list="list-attribute-0" class="form-control" />

                                            <input type="hidden" name="product_attribute[<?= $pa['id'] ?>][attribute_id]" value="<?= $pa['id'] ?>" id="input-attribute-id-<?= $pa['id'] ?>" />

                                            <datalist id="list-attribute-0">
                                                <?php foreach ($attribute as $att) : ?>
                                                    <option><?= $att['attributename'] ?></option>
                                                <?php endforeach ?>
                                            </datalist>
                                        </td>
                                        ...
                                        <td class="text-start">
                                            <div class="input-group">
                                                ...
                                                <textarea name="product_attribute[<?= $pa['id'] ?>][product_attribute_description]" rows="5" placeholder="Text" id="input-text-<?= $pa['id'] ?>" class="form-control"><?= $pa['attributevalue'] ?></textarea>

                                            </div>
                                        </td>
                                        <td id="delete_attribute" class="text-end"><button type="button" onclick="$('#attribute-row-<?= $pa['id'] ?>').remove();" data-bs-toggle="tooltip" title="Remove" class="btn btn-danger"><i class="fas fa-minus-circle"></i></button></td>
                                    </tr>
                                <?php endforeach ?>
                            </tbody>
                            <tfoot>
                                ...
                                ?>
                                    <tr id="special-row-<?= $sp['id'] ?>" class="<?= $sp['id'] ?>">
                                        <td class="text-start">
                                            ...
                                            <select name="product_special[special-row-<?= $sp['id'] ?>][customer_group_id]" class="form-select">
                                                <option value="1">Default</option>
                                            </select>

                                            <input type="hidden" name="specialid" value="<?= $sp['id'] ?>" />
                                            
                                            <input type="hidden" name="product_special[special-row-<?= $sp['id'] ?>][product_special_id]" value="<?= $sp['id'] ?>" />

                                            <input type="hidden" name="product_special[special-row-<?= $sp['id'] ?>][product_id]" value="<?= $sp['productid'] ?>" /></td>
...
<script>
    $(document).ready(function() {
        ...
        $('#button-attribute').on('click', function() {
            ...
            html += '    <input type="text" name="product_attribute_add[' + attribute_row + '][name]" value="" placeholder="Attribute" id="input-attribute-' + attribute_row + '" list="list-attribute-0" class="form-control"/>';
            html += '    <input type="hidden" name="product_attribute_add[' + attribute_row + '][attribute_id]" value="" id="input-attribute-id-' + attribute_row + '"/>';
            ...
            html += '  <textarea name="product_attribute_add[' + attribute_row + '][product_attribute_description]" rows="5" placeholder="Text" id="input-text-' + attribute_row + '" class="form-control"></textarea>';
            html += '</div>';
            html += '  </td>';
        ...
        $(document).on('click', '#delete_attribute', function(e) {
            ...
            $.ajax({
                method: "POST",
                url: "<?= base_url() . 'admin/deleteproductattribute' ?>",
                data: {
                    id: id,
                },
                success: function(data) {
